...
/**
 * Метаданные для некоторого типа ресурсов или какого-то конкретноо ресурса, к которому требуется
 ...
 * Ресурс допускает добвлять любые свойства, а также имеет несколько опциональных свойств с
 * предопределенными ключами: action, payload и type. <br/>
 * 1. <i>action</i> - все свойства с этим ключом описывают набор обощенных действий, который возможно выполнить
 * над ресурсом к которым требуется ограничить доступ. Все значения таких свойств имеют тип {@link Action}.<br/>
 * 2. <i>payload</i> - непосредственно объект, к которому регулируется доступ или любые другие данные. <br/>
 * 3. <i>type</i> - группа ресурсов объединенных вместе по некоторым признакам и рассматриваемых как единое целлое.
...
public class Resource {
    ...
    /**
     * Возвращает тип даного ресурса. Если тип не был указан - возвращает пустой Optional.
     * @return тип даного ресурса.
     */
     ...
     * @param type предпологваемый тип ресурса.
    ...
    /**
     * Проверяет - была ли добавленна полезная нагрузка для данного ресурса.
    ...
    /**
     * Проверяет - имеет ли полезная нагрузка заданный тип или супертип. Если полезная нагрузка не добавлялась
     * к этмоу ресурсу - возвращает false.
    ...
    /**
     * Проверяет - равняется ли полезная нагрукза указанному объекту. Если полезная нагрузка не задана -
     * возвращает false.
    ...
    /**
     * Возвращает полезную нагрузку предварително преобразованную к заданному типу. Если полезная нагрузка не была
     * задана - возвращает пустой Optional.
     * @param type тип к которому преобразуется полезная нагрузка.
     * @return полезную нагрузку преобразованную к указаннмоу типу.
     * @throws ClassCastException если не удалось преобразовать полезную нагрузку к указаннмоу типу.
     */
    ...
    /**
     * Возвращает список всех действий, котоыре можно выполнить над ресурсом.
     * @return список всех действий, котоыре можно выполнить над ресурсом.
     */
     ...
     * может использоваться для создания разных и независмых(с точки зрения разделяемых данных) ресурсов.
     */
    public static class Builder {
        ...
        /**
         * Устанавливат полезную нагрузку.
